...
public class DbHelper {
    ...
    public static boolean updateReminderSuccessfully(Context context, Reminder r)
    {
        try {
            sampleDB = context
                    .openOrCreateDatabase(dbName,
                            context.MODE_PRIVATE, null);
            sampleDB.execSQL("UPDATE " + reminderTable + " SET " +
                    "title ='"+ r.getTitle() +"', " +
                    "description ='"+ r.getDescription() +"', " +
                    "dueDate ='"+ r.getDueDate() +"', " +
                    "completed ='" + r.isCompleted() + "' " +
                    "WHERE reminderID = " + r.getId());
    ...
    public static Reminder selectSingleResult(Context context, String id)
    {
        try {
            ...
            String[] args={id};
            Cursor c = sampleDB.rawQuery("SELECT * FROM "
                    + reminderTable + " WHERE reminderID = ?", args);
            // If Cursor is valid
            if (c != null) {
                // Move cursor to first row
                if (c.moveToFirst()) {
